...
def upload_excel_file():
    ...
    print(f"Ruta del archivo subido: {file_path}")
    ...
    for index, row in df.iterrows():
        if row['material'] == '300 Series SS':
            ...
            if row['amine_type'] in ['MEA', 'DEA']:
                cr_value = buscar_cr_mea_dea(row['acid_gas_loading'], row['temperature'], row['velocity'], row['HSAS'])
            elif row['amine_type'] in ['MDEA']:
                cr_value = buscar_cr_mdea(row['acid_gas_loading'], row['temperature'], row['velocity'], row['HSAS'])
        else:
            cr_value = 'Unknown'
...
def buscar_cr_mdea(acid_gas_loading, temperature, velocity, hsas):
    """
    ...
    rango_datos_lower = None
    rango_datos_upper = None

    if acid_gas_loading < 0.1:
        rango_datos = data["acid_gas_loading"][rango_acid_gas_loading]
    else:
        if str(lower_agl) in data["acid_gas_loading"]:
            rango_datos_lower = data["acid_gas_loading"][str(lower_agl)]
        if str(upper_agl) in data["acid_gas_loading"]:
    ...
    cr_lower_hsas_lower_temp = get_cr(datos_hsas_lower, lower_temp, velocity) if acid_gas_loading >= 0.1 else None
    cr_lower_hsas_upper_temp = get_cr(datos_hsas_lower, upper_temp, velocity) if acid_gas_loading >= 0.1 else None
    cr_upper_hsas_lower_temp = get_cr(datos_hsas_upper, lower_temp, velocity) if acid_gas_loading >= 0.1 else None
    cr_upper_hsas_upper_temp = get_cr(datos_hsas_upper, upper_temp, velocity) if acid_gas_loading >= 0.1 else None
    ...
    cr_lower_hsas_lower_temp = float(cr_lower_hsas_lower_temp) if cr_lower_hsas_lower_temp is not None else None
    cr_lower_hsas_upper_temp = float(cr_lower_hsas_upper_temp) if cr_lower_hsas_upper_temp is not None else None
    cr_upper_hsas_lower_temp = float(cr_upper_hsas_lower_temp) if cr_upper_hsas_lower_temp is not None else None
    cr_upper_hsas_upper_temp = float(cr_upper_hsas_upper_temp) if cr_upper_hsas_upper_temp is not None else None

    cr_lower_temp = linear_interpolate(temperature, lower_temp, upper_temp, cr_lower_hsas_lower_temp, cr_lower_hsas_upper_temp) if cr_lower_hsas_lower_temp is not None and cr_lower_hsas_upper_temp is not None else None
    cr_upper_temp = linear_interpolate(temperature, lower_temp, upper_temp, cr_upper_hsas_lower_temp, cr_upper_hsas_upper_temp) if cr_upper_hsas_lower_temp is not None and cr_upper_hsas_upper_temp is not None else None
    cr = linear_interpolate(hsas, lower_hsas, upper_hsas, cr_lower_temp, cr_upper_temp) if cr_lower_temp is not None and cr_upper_temp is not None else None

    # return {"cr": cr}
    return cr

def buscar_cr_mea_dea(acid_gas_loading, temperature, velocity, hsas):
    """
    Function to find the CR value in a JSON file based on input parameters.
...
def procesar():
    print('procesar begin')
    data = request.json
    acid_gas_loading = data.get('acid_gas_loading')
    temperature = data.get('temperature')
    velocity = data.get('velocity')
    hsas = data.get('hsas')
    typeAmine = data.get('typeAmine')
    print(acid_gas_loading, temperature, velocity, hsas, typeAmine)

    if acid_gas_loading is None or temperature is None or velocity is None or hsas is None:
        return jsonify({"error": "Missing data"}), 400

    if typeAmine in ['MEA', 'DEA']:
        cr_value = buscar_cr_mea_dea(acid_gas_loading, temperature, velocity, hsas)
    elif typeAmine == 'MDEA':
        cr_value = buscar_cr_mdea(acid_gas_loading, temperature, velocity, hsas)
    else:
        print('Error typeAmine')

    print(cr_value)  
    return jsonify({"cr": cr_value})
